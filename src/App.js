...
const pocatecniBanky = [
  { id: 1, nazev: "Komerční banka", sazba: 6.29, aktivni: false, poplatekZaZpracovani: 0 },
  { id: 2, nazev: "ČSOB", sazba: 6.29, aktivni: false, poplatekZaZpracovani: 0 },
  { id: 3, nazev: "Česká spořitelna", sazba: 6.29, aktivni: false, poplatekZaZpracovani: 0 },
  { id: 4, nazev: "UniCredit Bank", sazba: 6.29, aktivni: false, poplatekZaZpracovani: 0 },
  { id: 5, nazev: "Raiffeisenbank", sazba: 6.29, aktivni: false, poplatekZaZpracovani: 0 },
  { id: 6, nazev: "mBank", sazba: 6.29, aktivni: false, poplatekZaZpracovani: 0 },
  { id: 7, nazev: "Oberbank", sazba: 6.29, aktivni: false, poplatekZaZpracovani: 0 },
];
  ...
  const exportujDoPDF = async () => {
   ...
   doc.addImage(img, 'PNG', 180, 10, 20, 20); // x, y, width, height
    ...
    if (refGrafu.current && vysledky.length > 0) {
      ...
      if (doc.lastAutoTable.finalY + imgVyska + 10 < vyskaPDF) {
        doc.addImage(imgData, 'PNG', 15, doc.lastAutoTable.finalY + 10, imgSirka, imgVyska);
      } else {
        doc.addPage();
        doc.addImage(imgData, 'PNG', 15, 20, imgSirka, imgVyska);
      }
    }
    ...
    // Přidání zápatí na všechny stránky
    ...
    if (doplnujiciInformace.trim() !== '') {
      doc.addPage();
      doc.setFontSize(12);
      doc.text("Doplňující informace:", 14, 20);
      doc.setFontSize(10);
      const rozdelenyText = doc.splitTextToSize(doplnujiciInformace, doc.internal.pageSize.width - 28);
      doc.text(rozdelenyText, 14, 30);
    }
          ...
          <div className="parametry-mrizka">
            <div className="skupina-vstupu">
              ...
              <input 
                ...
                type="number" 
                value={vyseUveru} 
                onChange={(e) => nastavVysiUveru(Number(e.target.value))} 
              />
            </div>
            <div className="skupina-vstupu">
